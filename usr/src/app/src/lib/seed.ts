...
// This function seeds the local discovery collections with predefined data for Vienna.
...
export async function seedViennaActivities() {
  ...
  const collectionsToSeed = ['couplesActivities', 'friendsActivities', 'meetActivities'];
  
  console.log("Starting to seed Vienna activities into all local discovery modules...");

  for (const collectionName of collectionsToSeed) {
    const activitiesCollection = firestore.collection(collectionName);
    console.log(`Seeding into ${collectionName}...`);
    ...
    for (const activityData of viennaDateIdeas) {
      try {
        ...
        if (querySnapshot.empty) {
          // Activity does not exist in this collection, so let's add it.
          ...
          console.log(`  -> Successfully seeded: "${activityData.name}"`);
        }
      } catch (error) {
        console.error(`  -> Error seeding activity "${activityData.name}" into ${collectionName}:`, error);
      }
    }
  }
