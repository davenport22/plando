...
async function seedDatabase() {
  ...
  console.log("Checking database for existing activities...");
  const activitiesCollection = firestore.collection('activities');
  
  // 1. Fetch existing system-generated activities
  const existingActivitiesSnapshot = await activitiesCollection.where('createdBy', '==', 'system').get();
  const existingActivityNames = new Set(existingActivitiesSnapshot.docs.map(doc => doc.data().name));
  console.log(`Found ${existingActivityNames.size} existing system-generated activities.`);

  // 2. Determine which activities are new
  const newActivitiesToSeed = allActivities.filter(activity => !existingActivityNames.has(activity.name));

  if (newActivitiesToSeed.length === 0) {
    console.log("Database is already up-to-date. No new activities to seed.");
    return;
  }
  
  console.log(`Found ${newActivitiesToSeed.length} new activities to add. Starting seed process...`);

  // 3. Process only the new activities
  const writeBatch = firestore.batch();
  let count = 0;

  for (const activityData of newActivitiesToSeed) {
    count++;
    console.log(`[${count}/${newActivitiesToSeed.length}] Generating image for new activity: ${activityData.name}...`);
    try {
  ...
  if (count > 0) {
      console.log("Committing new activities to the database...");
      await writeBatch.commit();
      console.log(`Successfully added ${count} new activities.`);
  } else {
      console.log("No new activities were committed.");
  }
